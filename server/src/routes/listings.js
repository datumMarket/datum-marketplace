...
router.get('/listings', ah(async (req, res) => {
  ...
  // When a query is present, relevance leads and the requested sort breaks ties.
  ...
  const rows = db.prepare(`SELECT * FROM listings WHERE ${where.join(' AND ')} ORDER BY ${relevance}${order} LIMIT ? OFFSET ?`)
